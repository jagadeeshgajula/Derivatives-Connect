...
public:
	...
	virtual double round( double v,  int nd ) const;
...
double
RoundingConvNearest::round( double v,  int nd ) const
{
	//if (!IRDConnect::RoundingConv::getRoundingFlag()) return v;

	// In a lot of cases, the same number is passed in for many times.
	static double oldV = 0.0;
	static int oldNd = 0.0;
...
RoundingConv( String( "SCINOTANEAR" ) )
{
}
...
double
RoundingConvSciNotaNearest::round( double v,  int nd ) const
{
...
double
RoundingConvDown::round( double v,  int nd ) const
{
	//if (!RoundingConv::getRoundingFlag()) return v;

	// In a lot of cases, the same number is passed in for many times.
	static double oldV = 0.0;
	static double oldNd = 0.0;
	static double oldResult = 0.0;
...
const RoundingConv *
RoundingConv::find( const std::string & name )
{
    RoundConvMap::const_iterator it = getRoundConvMap().find( name ) ;
    if ( it != getRoundConvMap().end() )
        return it->second ;
	return 0;
}

std::map<std::string, const  RoundingConv *> & RoundingConv::getRoundConvMap ()
{
     static RoundConvMap roundConvMap ;
     static bool inited1 = false ;
     if ( !inited1 ) {
    	 roundConvMap.insert ( RoundConvMap::value_type("_nearest", new RoundingConvNearest )) ;
    	 roundConvMap.insert ( RoundConvMap::value_type("_sciNotaNearest", new RoundingConvSciNotaNearest  )) ;
    	 roundConvMap.insert ( RoundConvMap::value_type("_down", new RoundingConvDown )) ;
