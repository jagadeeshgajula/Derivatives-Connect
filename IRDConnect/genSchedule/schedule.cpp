...
*/
	...
	Util::setValuationDate( Date::todaysDate());
	...
	Date cDate("2013/09/26","Y/m/d");
	Util::setCurveDate(cDate) ;

	cout << "Curve Date " << Util::getCurveDate() << endl;

	//boost::shared_ptr<IRDConnect::Curve>  curvePtr = CurveCache::instance().find(Util::getCurveDate(), "GBPLIBOR") ; //(dates , zcdfs ) ;
	//curvePtr->print();

	//if (  curvePtr->getZcdf(Util::getCurveDate()) == 1 )
		//cerr << "Day 1 ZCDF must be equal to 1 - curve failed : " <<  curvePtr->getZcdf(Util::getCurveDate()) << endl;
	...
	SwapTicketList swapTicketList;
		...
		*/
		...
		//cout << "Writing a XML here " << endl;
				...
				//cout << "DV 01 : " << setprecision(15) << (*sd)->getFirstPayLeg()->calculateDV01("ALL", 0.0001) << ": "
					//	<<(*sd)->getFirstReceiveLeg()->calculateDV01("ALL", 0.0001) << endl;
				cout << "DV 01 : " << setprecision(15) << (*sd)->getFirstPayLeg()->calculateDV01() << ": "
										<<(*sd)->getFirstReceiveLeg()->calculateDV01() << endl;

				cout << "Theta : " << setprecision(15) << (*sd)->getFirstPayLeg()->calculateTheta() << ": "
										<<(*sd)->getFirstReceiveLeg()->calculateTheta() << endl;
		...
		// FraTicket HERE
			...
			for ( ; fra != fraTicketList.end () ; ++fra )
			{
				...
				//(*fra)->setAdditionalPayments(fee2);
					...
					logFile << "FraTicket - TOTAL PV is : " << setprecision(15) << (*fra)->calculatePV() << endl;
		}
			fraTicketList.clear();
			...
			logFile << "Processing CAP Ticket " << endl;
			...
			for ( ; cf != cfTicketList.end () ; ++cf )
			{
				...
				if (  ! (*cf)->genCashFlows() )
				{
					...
					logFile << "CapFloorTicket - TOTAL PV is : " << setprecision(15) << (*cf)->calculatePV() << endl;
			}
