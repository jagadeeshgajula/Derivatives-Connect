...
namespace IRDConnect {
    ...
    {
    	serialNumber_ = date.serialNumber();
    ...
    Date::Date(Day d, int m, Year y) {
            ...
            bool leap = isLeap(y);
            Day len = monthLength(Month(m),leap), offset = monthOffset(Month(m),leap);
            QL_REQUIRE(d <= len && d > 0,
                       "day outside month (" << Integer(m) << ") day-range "
                       << "[1," << len << "]");
    ...
    // below copy constructor takes string for the date

Date::Date(const char * str , std::string pic)
{
	...
	if ( str ) {
		 ...
		 if ( pic == "Y/m/d")
		 {
			 ...
			 serialNumber_ = Date(day,month,year).serialNumber();
		...
		if ( pic == "d-M-Y")
		{
			...
			sscanf(str, "%d-%s-%d", &day,mTmp,&year);
		}
		else if ( pic == "d/M/Y" )
		{
			sscanf(str, "%d/%s/%d", &day,mTmp,&year);
...
*/
		...
		serialNumber_ = Date(day,Month(month),year).serialNumber();
...
std::string Date::string()
{
	...
	//cout << "day - Month - year " << dd << " - " << mth << " - " << yy << endl;

	if (mth < 10 && dd < 10 )
		sprintf(str, "0%d/0%d/%d", dd, mth,yy);
	else if ( mth < 10 && dd > 9 )
		sprintf(str, "%d/0%d/%d", dd,mth,yy);
	else if ( mth > 9 && dd < 10 )
			sprintf(str, "0%d/%d/%d", dd,mth,yy);
	else if ( mth > 9 && dd > 9 )
			sprintf(str, "%d/%d/%d", dd,mth,yy);
...
std::string Date::string(std::string dateFormat)
{
	char str[100];
	int dd = dayOfMonth();
	int yy = year();
	...
	if (mth < 10 && dd < 10 && dateFormat == "Ymd")
		sprintf(str, "%d0%d0%d", yy, mth,dd);
	else if ( mth < 10 && dd > 9 && dateFormat == "Ymd" )
		sprintf(str, "%d0%d%d", yy,mth,dd);
	else if ( mth > 9 && dd < 10 && dateFormat == "Ymd" )
			sprintf(str, "%d%d0%d", yy,mth,dd);
	else if ( mth > 9 && dd > 9 && dateFormat == "Ymd" )
			sprintf(str, "%d%d%d", yy,mth,dd);
...
Month Date::month() const {
    ...
    void Date::setValuationDate(Date date)
    {
    	_valuationDate = date;

    }

    void Date::setValuationDate(Day d, int m, Year y)
    {
        	_valuationDate = Date(d,m,y);
    ...
    namespace IRDdetail {
        ...
        std::ostream& operator<<(std::ostream& out,
            ...
            if (d == Date()) {
                ...
                out << d.month() << " ";
                out << IRDio::ordinal(d.dayOfMonth()) << ", ";
                out << d.year();
            }
            return out;
        }
    ...
    namespace IRDio {
        ...
        IRDdetail::short_date_holder short_date(const Date& d) {
            return IRDdetail::short_date_holder(d);
        }

        IRDdetail::long_date_holder long_date(const Date& d) {
            return IRDdetail::long_date_holder(d);
        }

        IRDdetail::iso_date_holder iso_date(const Date& d) {
            return IRDdetail::iso_date_holder(d);
        }
